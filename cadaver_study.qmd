...
Given that, the cycles can be averaged together to reduce the variability.
...
Test if the plate type significantly effects the stiffness.
...
anova_results |> filter(!is.na(p.value))
```
...
anova_results |> 
  select(location, term, df, meansq) |> 
  tidyr::pivot_wider(names_from = term, values_from = c(df, meansq)) |> 
  summarise(groups = 1 + df_plate, 
            n = 1 + (df_Residuals / groups), 
            b.var = meansq_Residuals, 
            w.var = meansq_plate, 
            sig.level = 0.05, 
            power = 0.8) |> 
  group_by(location) |>
  summarise(
    n_in_test = n,
    n_required = power.anova.test(groups=groups, 
                                  between.var=b.var, 
                                  within.var=w.var, 
                                  sig.level=sig.level, 
                                  power=power)$n
...
The number of specimens was enough to determine a difference for the position
and distal locations, but not the proximal location, 
due to the greater variation in those results.
...
# These options only use one equivalence bound for all locations - not accurate.
# results |>
#   tidyr::pivot_wider(names_from = plate, values_from = value) |>
#   group_nest(location) |>
#   mutate(
#     res = purrr::map(data,
#                      ~ TOSTER::t_TOST(.x$long, .x$short, paired = TRUE, eqb = 10) |>
#                        tidy_tost()
#                      )
#     ) |>
#   tidyr::unnest(res) |>
...
#   group_by(location) |> 
#   group_modify(
#     ~ TOSTER::t_TOST(.x$long, .x$short, paired = TRUE, eqb = 10) |> tidy_tost()
#   )

# Apply a different equivalcnce bound per position.
results |> 
  group_by(location) |> 
  group_modify(
...
Given the large variation in results, it is likely that many specimens would be 
needed to make a determination of equivalence.
A power test shows this to be the case.
...
Given the large number of specimens required at even a `r eqb_print` difference
it is reasonable to conclude that inter-specimen (or patient) differences are
greater than any inherent difference between plates.