...
This file compares the results from Experiment 2 on the 3D printed models
with the same models and plates, just with the lag screws removed.

Experiment 2 used 3D printed bone surogates, designed to match the underside 
of each plate design. 
The loading apparatus was designed to put the point of load application at the
same distance from the 'fusion' site, to mathc the bending moment and shear 
between the two plates being tested (long and short).

In addition, the short plates have a pair of lag screws that cross the 'fusion'
site.
We hypothesised that removing these screws would reduce the stiffness of the 
construct.
...
The code above is the same as the previous `Experiment 2` analysis.
Now load the data for the tests with no lag screw.

The no-lag-screw condition is coded in the file name, but add it to the plate 
name so they can be easily distnguished.
...
The no-lag-screw tests were done after all the reps of the lag screw tests.
`Experiment 2` showed no effect of repeated testing for this model, 
but the last rep of the long and lag-screw short plates were combined with 
the last rep of the no-lag-screw short plates to ensure they were as comparable
as possible.
...
These have been extracted from the raw files, compiled into a single table,
...
:::{.callout-note}
**Plate 1 is missing the load & position data from VIC-3D,**
**probably because the analog capture was not exported.**
**Fix this before final analysis.**
:::
